...
async def get_admins(message: types.Message, bot: Bot):
    ...
    if message.from_user.id in admins_list:
        ...
        try:
            ...
            print(f"Сообщение {message.message_id} удалено: {result}")
        except Exception as e:
            print(f"Ошибка при удалении сообщения {message.message_id}: {e}")
    
    # Проверка: отправлено ли сообщение от имени группы
    is_from_group = message.sender_chat is not None
    if is_from_group:
        print(f"Сообщение от имени группы: {message.sender_chat.title} (ID: {message.sender_chat.id})")
        try:
            result = await message.delete()
            print(f"Сообщение {message.message_id} удалено: {result}")
        except Exception as e:
        ...
        print(f"Сообщение от пользователя: {message.from_user.id}")
    print(admins_list)
